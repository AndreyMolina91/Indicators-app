...
export class BitcoinChartComponent implements OnInit {
  ...
  indicatorBitcoinModel: IndicatorModel = {
    version: '',
    autor: '',
    codigo: '',
    nombre: '',
  ...
  ngOnInit(): void {
    ...
    this.services.getIndicatorsBitcoin().subscribe((res: any) => {
      ...
      console.log(this.indicatorBitcoinModel);

      //INICIO DEL CHART
      Highcharts.chart('containerBitcoin', {
        chart: {
          styledMode: true,
          type: 'areaspline'
        },
  ...
  checkRecord(): void {
    this.services.getIndicatorsBitcoin().subscribe((data: any) => {
